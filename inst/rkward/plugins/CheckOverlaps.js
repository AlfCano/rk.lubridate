...
function preview(){
	var d = getValue("overlap_date"); var i = getValue("overlap_int"); var r_command = d + " %within% " + i; echo("preview_data <- data.frame(Within=" + r_command + ");\n");
}
...
var d = getValue("overlap_date"); var i = getValue("overlap_int"); var r_command = d + " %within% " + i; echo("is_within <- " + r_command + ";\n");
}
...
function printout(is_preview){
	...
	if(!is_preview) {
		new Header(i18n("Check Overlaps results")).print();	
	}
	...
	if(!is_preview) {
		...
		if(saveResultActive) {
			echo(".GlobalEnv$" + saveResult + " <- is_within\n");
		}	
	}
